...
class TestConvert:
    ...
    def testConvert(self, fixedseed, states, inputs, outputs):
        ...
        if (ssOriginal.nstates != ssTransformed.nstates) and verbose:
            print(
                "WARNING: state space dimension mismatch: %d versus %d"
        ...
        for inputNum in range(inputs):
            for outputNum in range(outputs):
                if verbose:
                    print("Checking input %d, output %d" % (inputNum, outputNum))
                ssorig_mag, ssorig_phase, ssorig_omega = bode(
                    _mimo2siso(ssOriginal, inputNum, outputNum), deg=False, plot=False
                )
                ...
                tforig_mag, tforig_phase, tforig_omega = bode(
                    tforig, ssorig_omega, deg=False, plot=False
                )

                tforig_real = tforig_mag * np.cos(tforig_phase)
                tforig_imag = tforig_mag * np.sin(tforig_phase)
                np.testing.assert_array_almost_equal(ssorig_real, tforig_real)
                np.testing.assert_array_almost_equal(ssorig_imag, tforig_imag)

                #
                # Make sure xform'd SS has same frequency response
                #
                ssxfrm_mag, ssxfrm_phase, ssxfrm_omega = bode(
                    _mimo2siso(ssTransformed, inputNum, outputNum),
                    ssorig_omega,
                    deg=False,
                    plot=False,
                )
                ...
                tfxfrm = tf(num, den)
                tfxfrm_mag, tfxfrm_phase, tfxfrm_omega = bode(
                    tfxfrm, ssorig_omega, deg=False, plot=False
                )
    ...
    def testSs2tfStaticSiso(self):
        ...
        np.testing.assert_array_equal([[[1.0]]], gsiso.den)
    ...
    def test_tf2ss_robustness(self):
        ...
        np.testing.assert_array_almost_equal(
            np.sort(sys1tf.pole()), np.sort(sys1ss.pole())
        )
        np.testing.assert_array_almost_equal(
            np.sort(sys2tf.pole()), np.sort(sys2ss.pole())
        )
