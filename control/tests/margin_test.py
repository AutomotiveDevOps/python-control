...
from control.margins import margin
from control.margins import phase_crossover_frequencies
from control.margins import stability_margins
...
@pytest.fixture(
    params=[
        ...
        (
            TransferFunction,
            ([1], [1, 2, 3, 4]),
            (2.0, inf, 0.4170, 1.7321, nan, 1.6620),
            ([2.0], [], [1.2500, 0.4170], [1.7321], [], [0.1690, 1.6620]),
        ),
        (
            StateSpace,
        ...
        (
            ...
            (
                ...
                / (s ** 2 / 10.0 ** 2 + 2 * 0.04 * s / 10.0 + 1)
            ),
            (2.2716, 97.5941, 0.5591, 10.0053, 0.0850, 9.9918),
            (
                [2.2716],
                [97.5941, -157.7844, 134.7359],
                [1.0381, 0.5591],
                [10.0053],
...
@pytest.mark.parametrize(
    ...
    [
        (([1], [1, 2, 3, 4]), [1.7325, 0.0], [-0.5, 0.25]),
        (([1], [1, 1]), [0.0], [1.0]),
        (([2], [1, 3, 3, 1]), [1.732, 0.0], [-0.25, 2.0]),
        (
            (np.array([3, 11, 3]) * 1e-4, [1.0, -2.7145, 2.4562, -0.7408], 0.1),
            [1.6235, 0.0],
            [-0.28598, 1.88889],
        ),
    ],
...
def test_frd():
    ...
    resp = 10 ** (gain / 20) * np.exp(1j * phase / (180.0 / np.pi))
    ...
    assert_allclose([pm], [44.55], atol=0.01)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        dict(
            sysname="type0", K=2.0, atol=1e-2, result=(4.000, 67.6058, 1.7322, 0.7663)
        ),
        dict(
            sysname="type1",
            K=1.0,
        ...
        dict(
            sysname="example21", K=1.0, atol=1e-2, result=(0.0100, -14.5640, 0, 0.0022)
        ),
        dict(
            sysname="example21",
            K=1000.0,
...
@pytest.mark.parametrize(
    ...
    [
        dict(
            ...
            atol=1e-3,
            result=(
                [0.01, 179.2931, 2.2798e4, 1.5946e07, 7.2477e08],
                [-14.5640],
                [0.2496],
...
def test_zmore_stability_margins(tsys_zmore):
    ...
    res = stability_margins(tsys_zmore["sys"] * tsys_zmore["K"], returnall=True)
    compare_allmargins(
        res, tsys_zmore["result"], atol=tsys_zmore["atol"], rtol=tsys_zmore["rtol"]
    )


@pytest.mark.parametrize(
    "cnum, cden, dt,ref,rtol",
    [
        (  # gh-465
            [2],
            [1, 3, 2, 0],
            ...
            2e-3,
        ),  # the gradient of the function reduces numerical precision
        (  # 2/(s+1)**3
            [2],
            [1, 3, 3, 1],
            0.1,
