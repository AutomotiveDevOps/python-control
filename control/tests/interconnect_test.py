...
def test_summing_junction(inputs, output, dimension, D):
    ...
    sum = ct.summing_junction(inputs=inputs, output=output, dimension=dimension)
...
def test_interconnect_implicit():
    ...
    # System definition
    P = ct.ss2io(
        ct.rss(2, 1, 1, strictly_proper=True), inputs="u", outputs="y", name="P"
    )
    ...
    Tio_sum = ct.interconnect((C, P, sumblk), inplist=["r"], outlist=["y"])
    ...
    Tio_sum = ct.interconnect((kp_io, ki_io, P, sumblk), inplist=["r"], outlist=["y"])
    np.testing.assert_almost_equal(Tio_sum.A, Tss.A)
    np.testing.assert_almost_equal(Tio_sum.B, Tss.B)
    np.testing.assert_almost_equal(Tio_sum.C, Tss.C)
    np.testing.assert_almost_equal(Tio_sum.D, Tss.D)
    ...
    with pytest.raises(ValueError, match="not unique"):
        Tio_sum = ct.interconnect((Cbad, P, sumblk), inplist=["r"], outlist=["y"])
    ...
    with pytest.raises(ValueError, match="could not find"):
        Tio_sum = ct.interconnect((C, P, sumblk), inplist=["x"], outlist=["y"])

    with pytest.raises(ValueError, match="could not find"):
        Tio_sum = ct.interconnect((C, P, sumblk), inplist=["r"], outlist=["x"])
