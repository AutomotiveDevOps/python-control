...
cases = {
    "scalar": (1, lambda dtype, v: dtype(v)),
    "scalar in 0d array": (1, lambda dtype, v: np.array(v, dtype=dtype)),
    "numpy array": ([1, 2], lambda dtype, v: np.array(v, dtype=dtype)),
    "list of scalar": (1, lambda dtype, v: [dtype(v)]),
    "list of scalars": ([1, 2], lambda dtype, v: [dtype(vi) for vi in v]),
    "list of list of list of scalar": (1, lambda dtype, v: [[[dtype(v)]]]),
    "list of list of list of scalars": (
        [[1, 1], [2, 2]],
        lambda dtype, v: [[[dtype(vi) for vi in vr] for vr in v]],
    ),
    "tuple of scalar": (1, lambda dtype, v: (dtype(v),)),
    "tuple of scalars": ([1, 2], lambda dtype, v: tuple(dtype(vi) for vi in v)),
    ...
    "tuple of tuples of numpy arrays": (
        [[[1, 1], [2, 2]], [[3, 3], [4, 4]]],
        lambda dtype, v: tuple(
            tuple(np.array(vr, dtype=dtype) for vr in vp) for vp in v
        ),
    ),
    "list of tuples of numpy arrays": (
        [[[1, 1], [2, 2]], [[3, 3], [4, 4]]],
        lambda dtype, v: [tuple(np.array(vr, dtype=dtype) for vr in vp) for vp in v],
    ),
    "list of lists of numpy arrays": (
        [[[1, 1], [2, 2]], [[3, 3], [4, 4]]],
        lambda dtype, v: [[np.array(vr, dtype=dtype) for vr in vp] for vp in v],
    ),
}
...
@pytest.mark.parametrize("badinput", [[[0.0, 1.0], [2.0, 3.0]], "a"])
