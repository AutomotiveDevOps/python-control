...
class TestControlMatlab:
    ...
    def MIMO_mats(self):
        ...
        B = array([[9.09, 0], [0, 0], [0, 9.09], [0, 0]])
        C = array([[0, 0.159, 0, 0], [0, 0, 0, 0.159]])
        D = zeros((2, 2))
        return A, B, C, D
    ...
    def test_dcgain_mimo(self, MIMO_mats):
        ...
        assert_array_almost_equal(
            gain1, array([[0.0269, 0.0], [0.0, 0.0269]]), decimal=4
        )
    ...
    @pytest.mark.skip(reason="skipping test_check_convert_shape, need to update test")
    def test_check_convert_shape(self):
        ...
        arr = _check_convert_array(array([1.0, 2, 3]), [(3,)], "Test: ")
        assert isinstance(arr, np.ndarray)
        assert not isinstance(arr, matrix)

        # Input is array, shape (3,), two legal shapes
        arr = _check_convert_array(array([1.0, 2, 3]), [(3,), (1, 3)], "Test: ")
        assert isinstance(arr, np.ndarray)
        assert not isinstance(arr, matrix)
        ...
        # Input is array, 2D, shape (1,3)
        arr = _check_convert_array(array([[1.0, 2, 3]]), [(4,), (1, "any")], "Test: ")
        assert isinstance(arr, np.ndarray)
        assert not isinstance(arr, matrix)

        # Input is array, 2D, shape (3,1)
        arr = _check_convert_array(
            array([[1.0], [2], [3]]), [(4,), ("any", 1)], "Test: "
        )
        assert isinstance(arr, np.ndarray)
        assert not isinstance(arr, matrix)
        ...
        arr = _check_convert_array([[1.0, 2, 3]], [(3,), (1, 3)], "Test: ")
        assert isinstance(arr, np.ndarray)
        assert not isinstance(arr, matrix)
        ...
        # Input is array, 2D, shape (1,3)
        arr = _check_convert_array(
            array([[1.0, 2, 3]]), [(3,), (1, 3)], "Test: ", squeeze=True
        )
        assert isinstance(arr, np.ndarray)
        assert not isinstance(arr, matrix)
        ...
        self.assertRaises(
            ...
            _check_convert_array(
                array([[min, max, all]]), [(3,), (1, 3)], "Test: ", squeeze=True
            ),
        )
        ...
        self.assertRaises(
            ValueError,
            _check_convert_array(array([1.0, 2, 3, 4]), [(3,), (1, 3)], "Test: "),
        )
    ...
    def test_lsim(self, SISO_mats, MIMO_mats):
        ...
        self.assertRaises(ValueError, lsim(sys, U=0, T=[0.0, 1.0, 2.0, 2.0, 3.0], x0=0))
        # show()
    ...
    def test_convert_MIMO_to_SISO(self, SISO_mats, MIMO_mats):
        ...
        sys_siso_00 = _mimo2siso(sys_mimo, input=0, output=0, warn_conversion=False)
        sys_siso_11 = _mimo2siso(sys_mimo, input=1, output=1, warn_conversion=False)
        # print("sys_siso_00 ---------------------------------------------")
        # print(sys_siso_00)
        # print("sys_siso_11 ---------------------------------------------")
        # print(sys_siso_11)
        ...
        Dm = array([[0, 0.02], [0.02, 0]])
        sys_mimo = ss(Am, Bm, Cm, Dm)

        sys_siso_01 = _mimo2siso(sys_mimo, input=0, output=1, warn_conversion=False)
        sys_siso_10 = _mimo2siso(sys_mimo, input=1, output=0, warn_conversion=False)
        # print("sys_siso_01 ---------------------------------------------")
        # print(sys_siso_01)
        # print("sys_siso_10 ---------------------------------------------")
        # print(sys_siso_10)
