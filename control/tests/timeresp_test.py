...
from control.timeresp import _default_time_vector
from control.timeresp import _ideal_tfinal_and_dt
from control.timeresp import forced_response
from control.timeresp import impulse_response
from control.timeresp import initial_response
...
class TestTimeresp:
    ...
    @pytest.fixture
    def siso_dtf0(self):
        T = TSys(TransferFunction([1.0], [1.0, 0.0], 1.0))
        T.t = np.arange(4)
        T.yimpulse = [0.0, 1.0, 0.0, 0.0]
        return T
    ...
    def mimo_dss1(self, mimo_ss1):
        ss1 = mimo_ss1.sys
        T = TSys(StateSpace(ss1.A, ss1.B, ss1.C, ss1.D, True))
        T.t = np.arange(0, 5, 0.2)
        return T
    ...
    def pole_cancellation(self):
        ...
        return TransferFunction([1.067e05, 5.791e04], [10.67, 1.067e05, 5.791e04])

    @pytest.fixture
    def no_pole_cancellation(self):
        return TransferFunction([1.881e06], [188.1, 1.881e06])
    ...
    @pytest.mark.parametrize(
        "kwargs",
        [{}, {"X0": 0}, {"X0": np.array([0, 0])}, {"X0": 0, "return_x": True},],
    )
    ...
    def test_step_info(self):
        ...
        np.testing.assert_allclose(
            [S[k] for k in Sk], [Strue[k] for k in Sktrue], rtol=rtol
        )

    def test_step_pole_cancellation(self, pole_cancellation, no_pole_cancellation):
        ...
        for key in step_info_no_cancellation:
            np.testing.assert_allclose(
                step_info_no_cancellation[key], step_info_cancellation[key], rtol=1e-4
            )
    ...
    @pytest.mark.skipif(
        StrictVersion(sp.__version__) < "1.3", reason="requires SciPy 1.3 or greater"
    )
    def test_discrete_time_impulse(self, siso_tf1):
        ...
        np.testing.assert_array_almost_equal(
            impulse_response(sys, t)[1], impulse_response(sysdt, t)[1]
        )
    ...
    @pytest.mark.parametrize("tsystem", ["siso_ss1", "siso_tf2"], indirect=True)
    ...
    @pytest.mark.parametrize(
        "u", [np.zeros((10,), dtype=np.float), 0]  # special algorithm
    )
    ...
    @pytest.mark.parametrize(
        "tsystem, useT",
        [("mimo_ss1", True), ("mimo_dss2", True), ("mimo_dss2", False)],
        indirect=["tsystem"],
    )
    ...
    @pytest.mark.parametrize(
        ...
        [
            (
                np.zeros((10,)),
                np.array([2.0, 3.0]),
                np.vstack([np.linspace(2, 5, 10), np.full((10,), 3)]),
            ),
            (
                np.ones((10,)),
                np.array([0.0, 0.0]),
                np.vstack([0.5 * np.linspace(0, 1, 10) ** 2, np.linspace(0, 1, 10)]),
            ),
            (
                np.linspace(0, 1, 10),
                np.array([0.0, 0.0]),
                np.vstack(
                    [np.linspace(0, 1, 10) ** 3 / 6.0, np.linspace(0, 1, 10) ** 2 / 2.0]
                ),
            ),
        ],
    ...
    @pytest.mark.parametrize(
        "tfsys, tfinal",
        [
            (TransferFunction(1, [1, 0.5]), 9.21034),  #  pole at 0.5
            (TransferFunction(1, [1, 0.5]).sample(0.1), 25),  # discrete pole at 0.5
            (TransferFunction(1, [1, 0.5, 0]), 25),
        ],
    )  # poles at 0.5 and 0
    ...
    @pytest.mark.parametrize("wn, zeta", [(10, 0), (100, 0), (100, 0.1)])
    ...
    def test_auto_generated_time_vector_dt_cont(self):
        ...
        wn = 100
        zeta = 0.1
        tfsys = TransferFunction(1, [1, 2 * zeta * wn, wn ** 2]).sample(0.1)
        tfinal, dt = _ideal_tfinal_and_dt(tfsys)
        np.testing.assert_almost_equal(dt, 0.1)
        T, _ = initial_response(tfsys)
        np.testing.assert_almost_equal(np.diff(T[:2]), [0.1])
    ...
    def test_default_timevector_functions_c(self, fun):
        ...
        tout, _ = fun(sys, 100)
        np.testing.assert_allclose(tout[-1], 100.0, atol=0.5)

    @pytest.mark.parametrize("fun", [step_response, impulse_response, initial_response])
    def test_default_timevector_functions_d(self, fun):
        """Test that functions can calculate the time vector automatically"""
        sys = TransferFunction(1, [1, 0.5, 0], 0.1)
    ...
    def test_time_vector_interpolation(self, siso_dtf2, squeeze):
        ...
        tout, yout = forced_response(sys, t, u, x0, interpolate=True, **squeezekw)
    ...
    @pytest.mark.parametrize(
        "nstate, nout, ninp, squeeze, shape1, shape2",
        [
    ...
    def test_squeeze(self, fcn, nstate, nout, ninp, squeeze, shape1, shape2):
        ...
        uvec = np.dot(np.ones((sys.ninputs, 1)), np.reshape(np.sin(tvec), (1, 8)))
        ...
        if isinstance(sys, StateSpace):
            # Check the states as well
            _, yvec, xvec = ct.step_response(sys, tvec, squeeze=squeeze, return_x=True)
            if sys.issiso():
                assert xvec.shape == (sys.nstates, 8)
            else:
                assert xvec.shape == (sys.nstates, sys.ninputs, 8)
        ...
        if isinstance(sys, ct.StateSpace):
            _, yvec, xvec = ct.forced_response(sys, tvec, uvec, 0, return_x=True)
            assert xvec.shape == (sys.nstates, 8)
        else:
    ...
    @pytest.mark.usefixtures("editsdefaults")
    @pytest.mark.parametrize(
        "nstate, nout, ninp, squeeze, shape",
        [
            [1, 1, 1, None, (8,)],
            [2, 1, 1, True, (8,)],
            [3, 1, 1, False, (1, 8)],
