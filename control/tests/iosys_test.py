...
class TestIOSys:
    ...
    def tsys(self):
        ...
        T.siso_linsys = ct.StateSpace([[-1, 1], [0, -2]], [[0], [1]], [[1, 0]], [[0]])

        # Create a multi input/multi output linear system
        T.mimo_linsys1 = ct.StateSpace(
            [[-1, 1], [0, -2]], [[1, 0], [0, 1]], [[1, 0], [0, 1]], np.zeros((2, 2))
        )

        # Create a multi input/multi output linear system
        T.mimo_linsys2 = ct.StateSpace(
            [[-1, 1], [0, -2]], [[0, 1], [1, 0]], [[1, 0], [0, 1]], np.zeros((2, 2))
        )
    ...
    def test_linear_iosys(self, tsys):
        ...
        np.testing.assert_allclose(lti_y, ios_y, atol=0.002, rtol=0.0)
    ...
    def test_ss2io(self, tsys):
        ...
        iosys_named = ct.ss2io(
            linsys, inputs="u", outputs="y", states=["x1", "x2"], name="iosys_named"
        )
    ...
    def test_nonlinear_iosys(self, tsys):
        ...
        nlupd = lambda t, x, u, params: np.reshape(
            np.dot(linsys.A, np.reshape(x, (-1, 1))) + np.dot(linsys.B, u), (-1,)
        )
        nlout = lambda t, x, u, params: np.reshape(
            np.dot(linsys.C, np.reshape(x, (-1, 1))) + np.dot(linsys.D, u), (-1,)
        )
    ...
    def test_linearize(self, tsys, kincar):
        ...
        np.testing.assert_array_almost_equal(linearized.B, [[1, 0], [0, 0], [0, 1]])
        np.testing.assert_array_almost_equal(linearized.C, [[1, 0, 0], [0, 1, 0]])
    ...
    def test_linearize_named_signals(self, kincar):
        ...
        linearized = kincar.linearize([0, 0, 0], [0, 0], copy=True, name="linearized")
    ...
    @pytest.mark.parametrize(
        ...
        [
            ...
            pytest.param(
                [["sys2.u[0]", ("sys1", "y[0]")]], [(0, 0)], [(1,)], id="mixed"
            ),
    ...
    def test_connect_spec_variants(self, tsys, connections, inplist, outlist):
        ...
        T, U = tsys.T, tsys.U
        X0 = np.concatenate((tsys.X0, tsys.X0))
        lti_t, lti_y, lti_x = ct.forced_response(linsys_series, T, U, X0, return_x=True)
    ...
    def test_static_nonlinearity(self, tsys):
        ...
        lti_t, lti_y, lti_x = ct.forced_response(linsys, T, Usat, X0, return_x=True)
        ios_t, ios_y, ios_x = ios.input_output_response(
    ...
    def test_algebraic_loop(self, tsys):
        ...
        nlios = ios.NonlinearIOSystem(
            None, lambda t, x, u, params: u * u, inputs=1, outputs=1
        )
        nlios1 = nlios.copy()
        nlios2 = nlios.copy()
        ...
        ios_t, ios_y = ios.input_output_response(nlios * lnios * nlios, T, U, X0)
        ...
        iosys = ios.InterconnectedSystem(
            ...
            [[1], [0, (1, 0, -1)]],  # feedback interconnection (sig to 0)
        ...
        iosys = ios.InterconnectedSystem(
            ...
            [[0, 1], [1, (0, 0, -1)]],  # feedback interconnection
            0,
            0,
        )
        args = (iosys, T, U)
        ...
        linsys = ct.StateSpace([[-1, 1], [0, -2]], [[0], [1]], [[1, 0]], [[1]])
        lnios = ios.LinearIOSystem(linsys)
        iosys = ios.InterconnectedSystem(
    ...
    def test_summer(self, tsys):
        ...
        np.testing.assert_allclose(ios_y, lin_y, atol=0.002, rtol=0.0)
    ...
    def test_feedback(self, tsys):
        ...
        ioslin = ios.LinearIOSystem(tsys.siso_linsys)
        nlios = ios.NonlinearIOSystem(
            None, lambda t, x, u, params: u, inputs=1, outputs=1
        )
        ...
        ios_t, ios_y = ios.input_output_response(iosys, T, U, X0)
        lti_t, lti_y = ct.forced_response(linsys, T, U, X0)
        np.testing.assert_allclose(ios_y, lti_y, atol=0.002, rtol=0.0)
    ...
    def test_discrete(self, tsys):
        ...
        # Create some linear and nonlinear systems to play with
        linsys = ct.StateSpace([[-1, 1], [0, -2]], [[0], [1]], [[1, 0]], [[0]], True)
        lnios = ios.LinearIOSystem(linsys)
        ...
        np.testing.assert_allclose(ios_t, lin_t, atol=0.002, rtol=0.0)
        np.testing.assert_allclose(ios_y, lin_y, atol=0.002, rtol=0.0)
    ...
    def test_find_eqpts(self, tsys):
        ...
        nlsys = ios.NonlinearIOSystem(predprey)
        xeq, ueq, result = ios.find_eqpt(nlsys, [1.6, 1.2], None, return_result=True)
        assert result.success
        np.testing.assert_array_almost_equal(xeq, [1.64705879, 1.17923874])
        np.testing.assert_array_almost_equal(nlsys._rhs(0, xeq, ueq), np.zeros((2,)))
        ...
        assert result.success
        np.testing.assert_array_almost_equal(nlsys._rhs(0, xeq, ueq), np.zeros((4,)))
        ...
        xeq, ueq, result = ios.find_eqpt(
            nlsys, [0, 0, 0, 0], [0.01, 4 * 9.8], y0=[0.1, 0.1], return_result=True
        )
        assert result.success
        np.testing.assert_array_almost_equal(
        ...
        # Unobservable system
        linsys = ct.StateSpace([[-1, 1], [0, -2]], [[0], [1]], [[0, 0]], [[0]])
        lnios = ios.LinearIOSystem(linsys)

        # If result is returned, user has to check
        xeq, ueq, result = ios.find_eqpt(lnios, [0, 0], [0], y0=[1], return_result=True)
    ...
    def test_params(self, tsys):
        ...
        lin_secord_local = ios.linearize(
            ios_secord_default, [0, 0], [0], params={"zeta": 0}
        )
        ...
        lin_secord_local = ios.linearize(
            ios_secord_update, [0, 0], [0], params={"zeta": 0, "omega0": 3}
        )
        w, v = np.linalg.eig(lin_secord_local.A)
        ...
        lin_series_override = ios.linearize(
            ios_series_default_local, [0, 0, 0, 0], [0], params={"zeta": 0, "omega0": 4}
        )
    ...
    def test_named_signals(self, tsys):
        ...
        ios_connect = ios.InterconnectedSystem(
            [sys2, sys1],
            connections=[[("sys1", "u[0]"), "sys2.y[0]"], ["sys1.u[1]", "sys2.y[1]"]],
    ...
    def test_lineariosys_statespace(self, tsys):
        ...
        np.testing.assert_array_equal(iosys_siso.pole(), tsys.siso_linsys.pole())
        omega = np.logspace(0.1, 10, 100)
    ...
    def test_docstring_example(self):
        P = ct.LinearIOSystem(ct.rss(2, 2, 2, strictly_proper=True), name="P")
        ...
        ss_eye = ct.StateSpace([], np.zeros((0, 2)), np.zeros((2, 0)), np.eye(2))
    ...
    def test_duplicates(self, tsys):
        ...
        # Duplicate names
        iosys_siso = ct.LinearIOSystem(tsys.siso_linsys)
        nlios1 = ios.NonlinearIOSystem(
            None, lambda t, x, u, params: u * u, inputs=1, outputs=1, name="sys"
        )
        nlios2 = ios.NonlinearIOSystem(
            None, lambda t, x, u, params: u * u, inputs=1, outputs=1, name="sys"
        )
        ...
        nlios1 = ios.NonlinearIOSystem(
            None, lambda t, x, u, params: u * u, inputs=1, outputs=1, name="nlios1"
        )
        nlios2 = ios.NonlinearIOSystem(
            None, lambda t, x, u, params: u * u, inputs=1, outputs=1, name="nlios2"
        )
...
def test_linear_interconnection():
    ...
    io_sys1 = ios.LinearIOSystem(
        ss_sys1, inputs=("u[0]", "u[1]"), outputs=("y[0]", "y[1]"), name="sys1"
    )
    io_sys2 = ios.LinearIOSystem(
        ss_sys2, inputs=("u[0]", "u[1]"), outputs=("y[0]", "y[1]"), name="sys2"
    )
    nl_sys2 = ios.NonlinearIOSystem(
        lambda t, x, u, params: np.array(
            np.dot(ss_sys2.A, np.reshape(x, (-1, 1)))
            + np.dot(ss_sys2.B, np.reshape(u, (-1, 1)))
        ).reshape((-1,)),
        lambda t, x, u, params: np.array(
            np.dot(ss_sys2.C, np.reshape(x, (-1, 1)))
            + np.dot(ss_sys2.D, np.reshape(u, (-1, 1)))
        ).reshape((-1,)),
        states=2,
        inputs=("u[0]", "u[1]"),
        outputs=("y[0]", "y[1]"),
    ...
    nl_connect = ios.interconnect(
        (io_sys1, nl_sys2),
        connections=[["sys1.u[1]", "sys2.y[0]"], ["sys2.u[0]", "sys1.y[1]"]],
        inplist=[["sys1.u[0]", "sys1.u[1]"], ["sys2.u[1]"]],
        outlist=[["sys1.y[0]", "-sys2.y[0]"], ["sys2.y[1]"], ["sys2.u[1]"]],
    )
...
def pvtol(t, x, u, params={}):
    ...
    m = params.get("m", 4.0)  # kg, system mass
...
def secord_update(t, x, u, params={}):
    ...
    return np.array([x[1], -2 * zeta * omega0 * x[1] - omega0 * omega0 * x[0] + u[0]])
