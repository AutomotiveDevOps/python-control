...
def dlqr(*args, **keywords):
    r"""Linear quadratic regulator design for discrete systems
...
def placep(A, B, P):
    ...
    A, B, ncont, indcont, nblk, z = _wrapper.ssxmc(
        n, m, A, n, B, wrka, wrk1, wrk2, iwrk, tol, mode
    )
    P = sort(P)
    wr = real(P)
    wi = imag(P)
    ...
    A, B, g, z, ierr, jpvt = _wrapper.polmc(
        A, B, g, wr, wi, z, indcont, nblk, rm1, rm2, rv1, rv2, rv3, rv4
    )
...
def bb_dare(A, B, Q, R):
    ...
    X, rcond, w, S, T = sb02od(nstates, ninputs, A, B, Q, R, "D")
