...
class MyFormatter(LogFormatterMathtext):
    def __call__(self, x, pos=None):
        if pos == 0:
            return ""  # pos=0 is the first tick
        else:
            return LogFormatterMathtext.__call__(self, x, pos)
...
def _PlotMag(freqvect, bodein, linetype="-", linewidth=0, axis=None):
    if callable(bodein.dBmag):
        ...
        myvect = bodein.dBmag
    return _PlotMatrixvsF(
        freqvect, myvect, linetype=linetype, linewidth=linewidth, axis=axis
    )


def _PlotPhase(freqvect, bodein, linetype="-", linewidth=0, axis=None):
    return _PlotMatrixvsF(
        freqvect, bodein.phase, linetype=linetype, linewidth=linewidth, axis=axis
    )
...
class TransferFunction(signal.lti):
    ...
    def __add__(self, other):
        if hasattr(other, "num") and hasattr(other, "den"):
            if (
                len(self.den.coeffs) == len(other.den.coeffs)
                and (self.den.coeffs == other.den.coeffs).all()
            ):
            ...
            raise ValueError(
                "do not know how to add TransferFunction and "
                + str(other)
                + " which is of type "
                + str(type(other))
            )
    ...
    def __pow__(self, expon):
        ...
        assert (
            expon >= 0
        ), "TransferFunction.__pow__ does not yet support negative exponents."
        out = 1.0
    ...
    def RootLocus(
        self, kvect, fig=None, fignum=1, clear=True, xlim=None, ylim=None, plotstr="-"
    ):
    ...
    def create_tvect(self, dt=None, maxt=None):
        ...
        assert dt is not None, (
            "You must either pass in a dt or call create_tvect on an instance with a"
            " self.dt already defined."
        )
        if maxt is None:
    ...
    def step_response(
        ...
        **kwargs,
        ...
        try:
            ystep = self.lsim(
                u, tvect, interp=interp
            )  # [1]#the outputs of lsim are (t, y,x)
        except:
        ...
        if fig is not None:
            ...
            if plotu:
                ...
                ax.plot(
                    tvect, u, fmts[0], linestyle="steps", **kwargs
                )  # assume step input wants 'steps' linestyle
                ofmt = fmts[1]
            else:
                ofmt = fmts[0]
    ...
    def swept_sine_response(
        self, maxf, minf=0.0, dt=None, maxt=None, deadtime=2.0, interp=0
    ):
    ...
    def c2d(self, dt=None, maxt=None, method="zoh", step_time=0.5, a=2.0):
        ...
        if method.lower() == "zoh":
            ...
            nz, dz = fit_discrete_response(
                ystep, myimp, self.den.order, self.den.order + 1
            )  # we want the numerator order to be one less than the denominator order - the denominator order +1 is the order of the denominator during a step response
            # multiply by (1-z^-1)
            nz2 = r_[nz, [0.0]]
            nzs = r_[[0.0], nz]
            nz3 = nz2 - nzs
...
class FirstOrderCompensator(Compensator):
    ...
    def __repr__(self):
        return TransferFunction.__repr__(
            self, labelstr="controls.FirstOrderCompensator"
        )

    def ToPSoC(self, dt=0.004):
        b, a = self.c2d(dt=dt)
        outstr = "v = {:f}*e{:+f}*ep{:+f}*vp;".format(b[0], b[1], -a[1])
        print("PSoC str:")
        print(outstr)
        return outstr
...
class Closed_Loop_System_with_Sat:
    def __init__(self, plant_tf, Kp, sat):
        self.plant_tf = plant_tf
        self.Kp = Kp
        self.sat = sat

    def lsim(self, u, t, X0=None, include_sat=True, returnall=0, lsim2=0, verbosity=0):
        dt = t[1] - t[0]
        if X0 is None:
        ...
        for n in range(1, N):
            ...
            if lsim2:
                t_nn, y_n, x_n = self.plant_tf.lsim2(
                    [v_n, v_n], [t_n, t_n + dt], X0=x_n[-1], returnall=1
                )
            else:
                t_nn, y_n, x_n = self.plant_tf.lsim(
                    [v_n, v_n], [t_n, t_n + dt], X0=x_n[-1], returnall=1
                )
...
def _getlinetype(ax=None):
    ...
    return {
        "color": colors[myind % len(colors)],
        "linestyle": mytypes[myind % len(mytypes)],
    }
