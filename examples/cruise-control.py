...
theta_hill = np.array(
    [
        0 if t <= 5 else 4.0 / 180.0 * pi * (t - 5) if t <= 6 else 4.0 / 180.0 * pi
        for t in T
    ]
)
...
for gear in range(5):
    ...
    plt.plot(
        v_range, [motor_torque(w) for w in omega_range], color="blue", linestyle="solid"
    )
...
cruise_pi = ct.InterconnectedSystem(
    (vehicle, control_pi),
    name="cruise",
    connections=(["vehicle.u", "control.u"], ["control.v", "vehicle.v"]),
    inplist=("control.vref", "vehicle.gear", "vehicle.theta"),
    outlist=("control.u", "vehicle.v"),
    outputs=["u", "v"],
)
...
def cruise_plot(
    sys, t, y, t_hill=5, vref=20, antiwindup=False, linetype="b-", subplots=[None, None]
):
...
theta_hill = [
    0 if t <= 8 else 4.0 / 180.0 * pi * (t - 8) if t <= 9 else 4.0 / 180.0 * pi
    for t in T
]
t, y = ct.input_output_response(
    cruise_sf,
...
vref = 20 * np.ones(T.shape)
theta_hill = [
    0 if t <= 5 else 6.0 / 180.0 * pi * (t - 5) if t <= 6 else 6.0 / 180.0 * pi
    for t in T
]
t, y = ct.input_output_response(
    cruise_pi, T, [vref, gear, theta_hill], X0, params={"kaw": 0}
)
...
t, y = ct.input_output_response(
    cruise_pi, T, [vref, gear, theta_hill], X0, params={"kaw": 2.0}
)
cruise_plot(cruise_pi, t, y, antiwindup=True)
